...
/*
    C++ Program to find the minimum and maximum value node in a  Binary Tree,
...
int findMaxElementNode(struct BinaryNode* root)
{
    if(root == nullptr)
        return 0;
    return max(root->data, max(findMaxElementNode(root->leftNode),findMaxElementNode(root->rightNode)));
}
...
int findMinElementNode(struct BinaryNode* root)
{
    if(root == nullptr)
        return INT_MAX;
    return min(root->data, min(findMinElementNode(root->leftNode),findMinElementNode(root->rightNode)));
}
...
int main()
{
    ...
    if(root != nullptr)
    {
        cout<<"\nThe Minimum Element Node of the Tree is : "<<findMinElementNode(root)<<endl;
        cout<<"\nThe Maximum Element Node of the Tree is : "<<findMaxElementNode(root)<<endl;
    }
    else
        cout<<"\n The Binary Tree is Empty. Cannot find the minimum and maximum node "<<endl;
