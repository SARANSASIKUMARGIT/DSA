...
/*
    C++ Program to print the number of Internal Nodes (Non-Leaf Nodes) in a Binary Tree,

    TIME COMPLEXITY  : O(N) for traversing N number of nodes recursively,
    SPACE COMPLEXITY : O(1) only constant space , no extra space taken
*/
...
int countInternalNodes(struct BinaryNode* root)                                 //function to count number of internal nodes in a binary Tree
{
    if(root == nullptr || (root->leftNode == nullptr && root->rightNode == nullptr))        // if the node is null or a leaf node then return 0
        return 0;
    return 1+(countInternalNodes(root->leftNode) + countInternalNodes(root->rightNode));    //if the node is not a leaf node, then add 1 + number of non-leaf nodes on left and right subtree recursively.
}
...
int main()
{
    ...
    cout<<"\nThe Number of Internal Nodes (Non-Leaf Nodes) of the Tree is : "<<countInternalNodes(root)<<endl;
