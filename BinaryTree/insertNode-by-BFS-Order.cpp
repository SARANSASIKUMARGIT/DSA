...
/*
    C++ Program to insert a node in the Binary Tree by Breadth-First Traversal Approach,
    This Program traverse every node of the tree by Level/Breadth-First Order and inserts the new node to the which has no left child or right child

    TIME COMPLEXITY  : O(N)  Linear Time Complexity for traversing N number of nodes during BFS insertion,
    SPACE COMPLEXITY : O(N)  for storing N number of nodes in queue during BFS Traversal
*/
...
int main()
{
    ...
    inOrderTraversal(root);                                    //printing the nodes of the tree by inorder traversal
