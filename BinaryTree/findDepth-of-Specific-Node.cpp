...
/*
    C++ Program to find the height of specific node in a Binary Tree by recursion
*/
...
int findDepthofNode(struct BinaryNode* root,int target)
{
    if(root == nullptr)
        return -1;
    if(root->data == target)
        return 1;
    int maxchildHeight = max(findDepthofNode(root->leftNode,target) , findDepthofNode(root->rightNode,target));
    if(maxchildHeight == -1)
        return -1;
...
int main()
{
    ...
    cout<<"Enter the node value to find depth : ";
    cin>>num;
    
    int depth = findDepthofNode(root,num);
    if(depth == -1)
        cout<<"Node "<<num<< " not found \n";
    else
        cout<<"Depth of Node "<<num<<" is "<<depth<<endl;
