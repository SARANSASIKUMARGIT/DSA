...
/*
    C++ Program to Print the Nodes in Diagonal Order in a Binary Tree


    TIME COMPLEXITY  : O(N) for traversing N number of nodes ,
    SPACE COMPLEXITY : O(N) where for storing values of N number of nodes in a result vector
*/
...
void printDiagonalNodes(struct BinaryNode* root)         //function to print the nodes in diagonal order of the binary tree
{
    ...
    while(!q.empty())
    {
        int size = q.size();                //calculating the number of current diagonal nodes 
        for(int i=1;i<=size;++i)
        {
            struct BinaryNode* currentNode = q.front();             //fetching current front node (first node of current diagonal) 
            q.pop();                                                //pop the node
            while(currentNode)
            {
                res.push(currentNode->data);                    //push the current node value to the result vector
                if(currentNode->leftNode)                       //if nodes has left child then it should be pushed to the queue for traversing it for the next diagonal
                    q.push(currentNode->leftNode);
                currentNode = currentNode->rightNode;           //move to right child of current node
            }
        }
    }

    cout<<"Elements Visited in Diagonal Order : ";
    for(int a : res)
        cout<<a<<" ";
    cout<<endl;
